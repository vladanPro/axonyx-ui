(() => {
  ...
  function codeButtonLabel(label) {
    return `${codeCopyIcon}<span>${label}</span>`;
  }
  ...
  function enhanceLegacyCodePre(pre) {
    ...
    button.innerHTML = codeButtonLabel('Copy');
  ...
  function bootCodeBlocks() {
    ...
    document.querySelectorAll('[data-ax-copy-code]').forEach((btn) => {
      ...
      if (!btn.innerHTML.trim() || btn.textContent.trim() === 'Copy') {
        btn.innerHTML = codeButtonLabel('Copy');
      }
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          await copyText(text);
          btn.innerHTML = codeButtonLabel('Copied');
          setTimeout(() => {
            btn.innerHTML = codeButtonLabel('Copy');
          }, 1200);
        } catch (err) {
          btn.innerHTML = codeButtonLabel('Error');
        }
      });
    });
  }
